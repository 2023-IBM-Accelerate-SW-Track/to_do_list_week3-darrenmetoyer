...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      color = "#ffffffff"
      if (new Date(due).isBefore(new Date())) {
        color = "red"
      }
      return (
        <Grid key={todo.id}>
          <Card style={{marginTop:10} } changeBackground:color  data-testid={todo.content}>
            ...
            <ListItemButton component="a" href="#simple-list">
              ...
              <ListItemText primary={todo.content} secondary={todo.due.date}/>
            </ListItemButton>
          </Card>
        </Grid>
      );
